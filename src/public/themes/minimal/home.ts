...
/** Render a single entry card for the sidebar list. */
function renderEntryCard(p: PageListItem): string {
...
/**
 * Render the homepage body.
 * - Left column: Journal Entries only
 * - Right column: Editors' Note (top) + About/Methodology cards (bottom)
 */
export function homeView(
  journalEntries: PageListItem[],
  staticPages: PageListItem[],
): string {
  const entryItems =
    journalEntries.length > 0
      ? journalEntries.map(renderEntryCard).join("\n")
      : `<li class="entry-card--empty"><p class="empty-state">No entries published yet.</p></li>`;

  const staticLinks = staticPages
    .map(
      (p) => `<li class="static-page-card">
    <a href="/${esc(p.slug)}" class="static-page-card__link">
      <span class="static-page-card__title">${esc(p.title)}</span>
      ${p.description ? `<span class="static-page-card__desc">${esc(p.description)}</span>` : ""}
    </a>
  </li>`,
    )
    .join("\n");
  ...
  <div class="home-main">
    <section class="editors-note">
      ...
      <div class="editors-note__body">
        ...
        <p>The Williams Radar is a technical signal system built on Bill Williams' Awesome Oscillator and Accelerator Oscillator. It scans 255 tickers across 13 sector ETFs. It identifies two configurations: S1 (early momentum building) and S2 (full confirmation — the entry signal). That is the entire model. No hidden layers. No proprietary secret.</p>
    ...
    ${
      staticLinks
        ? `<nav class="static-pages-nav" aria-label="Site sections">
