...
## Hardening pass (2026-04-27)

### Trust X-Forwarded-For only behind a proxy you control

The first version of the rate-limit + audit-log code honored `X-Forwarded-For` unconditionally as the client-IP key. An attacker hitting the server directly (or via any proxy that doesn't strip XFF) could rotate the header per request and never trip the limiter — security theater. **Fix**: gate XFF parsing on a `TRUST_PROXY=true` env flag. Default OFF means the server reads the raw socket address (or "unknown" in the test adapter). Set TRUST_PROXY=true ONLY when an upstream like Caddy/nginx overwrites XFF with the real connecting IP.

### Login limiters must not count successes

A 5-attempts/15-min limiter that counts every login — pass or fail — locks legitimate users out after a few normal sessions. The fix is a `skipOn(c)` callback that pops the bucket entry back off after the handler runs when the response indicates success (2xx for API, 2xx/3xx redirect for admin). Only failures and validation errors burn budget.

### Default `Secure` cookies ON, not "ON in prod"

Earlier the cookie attribute defaulted to `secure: NODE_ENV === "production"`. The footgun: any deploy that forgets to set `NODE_ENV` (a `tsx` staging script, a misconfigured systemd unit) silently downgrades the auth cookie to plain HTTP. **Inverted**: default is now ON unconditionally; `COOKIE_SECURE=false` is the only opt-out and exists for local HTTP dev only. The dev experience cost is one env var; the security benefit is "fail-closed" behavior under config drift.

### CSP `'unsafe-inline'` is the price you pay for inline event handlers

Two `<form onsubmit="confirm(...)">` handlers in admin views forced `script-src 'self' 'unsafe-inline'`. That defeats most of CSP's XSS protection — but **rewriting** the handlers to a hashed external script for an admin-only flow wasn't worth the churn at this stage. The CSP captures it as a known compromise (Security section in ARCHITECTURE.md). Future improvement: add `<script src="...confirm.js" integrity="sha256-..." />` and drop unsafe-inline.

### Magic-byte sniff after the allowlist, before the disk write

`file.type` from a multipart upload is client-controlled; the allowlist of MIME types only constrains what the client _says_ it's sending. Magic-byte validation must run server-side, on the buffered content, before `driver.write` and before any DB insert. The validation lives in `src/lib/mime-sniff.ts` — five signatures (PNG/JPEG/GIF/WebP/PDF) handled inline rather than pulling a `file-type` dep, since that gives a smaller blast radius. JPEG check requires the 4th byte to be `>= 0xC0` (a valid marker), not just any value following `FF D8 FF`.

### File-write rollback only matters if you never overwrite

Each upload's stored filename is `Date.now() + "-" + sanitize(file.name)`, so the millisecond prefix usually prevents collisions. The rollback (unlink-on-INSERT-failure) catches the rare race + actual UNIQUE constraint violations. Both API (`src/api/media.ts`) and admin form (`src/admin/router.ts`) paths now have it — easy to miss the second one if you only patch the API. Audit reviewers spotted this. Lesson: when adding a defense to one of two parallel routes, grep for the other.

### Test-only state on production modules: keep the export honest

`__resetRateLimitForTests()` is exported from production code so test files can clear the in-memory bucket map between tests. The double-underscore prefix and JSDoc make the contract obvious; a unit test that exercises the limiter realistically would otherwise carry buckets across test files via module-level state. The trade-off — a tiny bit of test-only API surface in prod code — is preferable to splitting modules just for testability.

### Idempotency rules retry decisions

`ApiClient.fetchWithRetry` initially retried any transient network error on any method. That's wrong for POST/PUT/DELETE: on `ECONNRESET` mid-request the server may have already accepted the request and the response was lost — retry would double-apply, creating duplicate pages or media. Now retries are GET-only via an explicit `retryable: boolean` flag. POST/PUT/DELETE either succeed cleanly or surface the network error to the caller for application-level retry with idempotency keys.
